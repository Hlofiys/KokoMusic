...
module.exports = {
  name: "seek",
  description: "Найти позицию песни",
  usage: "[время с/м/ч]",
  permissions: {
  ...
  run: async (client, message, args, { GuildDB }) => {
    ...
    if (!SeekTo)
      return client.sendTime(
        message.channel,
        `**Использование - **\`${GuildDB.prefix}seek <number s/m/h>\` \n**Пример - **\`${GuildDB.prefix}seek 2м 10с\``
      );
        ...
        options: [
            {
                ...
                *
                run: async (client, interaction, args, { GuildDB }) => {
                    ...
                    if (!player.queue.current.isSeekable) return client.sendTime(interaction, "❌ | **Не нашел эту песню**");
                    let SeekTo = client.ParseHumanTime(interaction.data.options[0].value);
                    if (!SeekTo) return client.sendTime(interaction, `**Использование - **\`${GuildDB.prefix}seek <number s/m/h>\` \n**Example -** \`${GuildDB.prefix}seek 2m 10s\``);
                    player.seek(SeekTo * 1000);
