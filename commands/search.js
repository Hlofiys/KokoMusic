...
module.exports = {
  name: "search",
  description: "Показывает список найденных песен по запросу",
  usage: "[песня]",
  permissions: {
  ...
  run: async (client, message, args, { GuildDB }) => {
    ...
    if (!SearchString)
      return client.sendTime(
        message.channel,
        `**Использование - **\`${GuildDB.prefix}search [query]\``
      );
    ...
    if (Searched.loadType == "NO_MATCHES")
      return client.sendTime(
        message.channel,
        "Не найдено совпадений с " + SearchString
      );
    else {
      ...
      let msg = await message.channel.send(
        "**Напиши количество песен которые ты хочешь воспроизвести. Истекает через `30 секкунд`.**"
      );
      ...
      let SongID = await message.channel
        ...
        .catch(() => {
          er = true;
          msg.edit(
            "**Время истекло. Напиши команду еще раз**"
          );
        });
      if (er) return;
      /**@type {Message} */
      ...
      if (!parseInt(SongIDmsg.content))
        return client.sendTime(
          message.channel,
          "Отправь правильный ID песни"
        );
      let Song = Searched.tracks[parseInt(SongIDmsg.content) - 1];
      if (!Song)
        return client.sendTime(
          message.channel,
          "Не найдено треков с даденным ID"
        );
    ...
    options: [
      {
        ...
        type: 3,
        required: true,
        description: "Напиши название песни или url которую ты хочешь воспроизвести",
      },
    ],
    /**
     *
    ...
    run: async (client, interaction, args, { GuildDB }) => {
      ...
      if (search.match(client.Lavasfy.spotifyPattern)) {
        ...
        try {
          ...
          return client.sendTime(
            interaction,
            `:x: | **Ошибка при поиске:** ${err.message}`
          );
        }
        switch (res.loadType) {
          case "NO_MATCHES":
          ...
          case "PLAYLIST_LOADED":
            ...
            return client.sendTime(
              interaction,
              `**Плейлист добавлен в очередь**: \n**${res.playlist.name}** \nВ очереди: **${res.playlistInfo.length} songs**`
            );
          case "SEARCH_RESULT":
            let max = 10,
              collected,
            ...
            const resultss = new MessageEmbed()
              .setDescription(
                `${results}\n\n\t**Напишите количество песен для воспроизведения**\n`
              )
              .setColor(client.botconfig.EmbedColor)
              .setAuthor(
            ...
            if (index < 0 || index > max - 1)
              return awaitchannel.send(
                `Число которые ты написал больше либо меньше всех найденных песен. Использование - \`(1-${max})\``
              );
